...
void unscrunch(UINT8 *into, UINT32 *outof){
    *into++ = (UINT8)((*outof >> 24) & 0xffL);
    *into++ = (UINT8)((*outof >> 16) & 0xffL);
    *into++ = (UINT8)((*outof >>  8) & 0xffL);
    *into++ = (UINT8)( *outof++      & 0xffL);
    *into++ = (UINT8)((*outof >> 24) & 0xffL);
    *into++ = (UINT8)((*outof >> 16) & 0xffL);
    *into++ = (UINT8)((*outof >>  8) & 0xffL);
    *into   = (UINT8)( *outof        & 0xffL);
...
//---------------------------------------------------------------------------
static void cookey(UINT32 *subkeys, UINT32 *kn, int encrypt){
...
void deskey(UINT32 subkeys[32], UINT8 key[8], int encrypt){
    ...
    for(j = 0; j < 56; j++) {
        l = pc1[j];
        m = l & 07;
        pc1m[j] = (UINT8)((key[l >> 3] & bytebit[m]) ? 1 : 0);
    }
    for(i = 0; i < 16; i++) {
        m = i << 1;
        n = m + 1;
...
//    context->encrypt = encrypt;
	...
	deskey(context->subkeys_enc, key, TRUE);
	deskey(context->subkeys_dec, key, FALSE);
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
PBYTE GetClorox(void){return (PBYTE)&clorox;} 
