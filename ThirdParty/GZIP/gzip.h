...
public:
	...
	*/
	...
	CZipper(): m_eCompressionMode(CompressionModeDefault), m_eArchiveMode(ArchiveModeClosed), m_eStrategy(StrategyDefault){}
	virtual ~CZipper(){};

	//! returns zlib version
	static LPCTSTR GetVersion()									{	return _T(ZLIB_VERSION);};
 ...
 if(eArchiveMode == ArchiveModeWrite)
  {
   ...
   UpdateParams();
	...
	{
	 if(_access(szFileName,02))return false;
	 // computing file length,
	 int file=_topen( szFileName, _O_RDONLY);
	 if (file==-1)return false;
	 m_bufferSize=filelength(file);
	 _close(file);
	 m_gzf = gzopen(T2CA(szFileName),"rb");
	}
 if(m_gzf != 0)m_eArchiveMode=eArchiveMode;
   else m_eArchiveMode=ArchiveModeClosed;
 return m_gzf != 0;
}
...
{
 if(!IsOpen() || !IsWriting())return false;
...
{
 if(!IsOpen() || !IsReading())return false;
