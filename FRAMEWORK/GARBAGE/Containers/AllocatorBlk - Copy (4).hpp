...
static uint CalcForIndex(size_t ObjIdx, size_t& Idx)           // Cannot pregenerate a table - may be too big!!!
{
 ...
 size_t cbidx = ((size_t)NMATH::sqrt((8 * (uoffs / PageSize)) + 1) - 1) / 2;   // FindBlockForOffset(uoffs);   // Ignoring headers and leftover bytes (will correct for them later)
 ...
 if constexpr ((PageSize - NHdrSize) % ObjSize)   // Will need corrections not only for the first header
  {
   ...
   SCVR size_t BlkLftRange = ObjSize / ((size_t)1 << ctz(ObjSize));    // Leftover bytes sequence repeat range (in blocks)   // Same as "ObjSize / std::gcd(PageSize, ObjSize)" 
 ...
 //  if(uoffs > xxx) 
  //   {
   ...
   uoffs += corra + CorrArr.Arr[1+aidx];          // Must recalc value from the array for first block
   //  }
   //   else
    //   {
    //     ucorr  = corra + 30;
 ...
 size_t unfix = (cblen - ucorr) / ObjSize;
 size_t ubidx = ObjIdx - unfix; 
 Idx = ubidx;
 return cbidx;
}
//-------------------------------------------------------------
