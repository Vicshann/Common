...
//                             // TODO: Page size must be greater than the object size!!!!!!!!!!!
template<size_t ObjLen, size_t PageLen=NPTM::MEMGRANSIZE, size_t FHdrLen=0, size_t NHdrLen=0> struct SBASUni: public SBASBase<AlignP2Frwd(PageLen,NPTM::MEMGRANSIZE), ObjLen, FHdrLen, NHdrLen, 14, (size_t)-1, SBASUni<ObjLen,PageLen,FHdrLen,NHdrLen> >   
{
...
// // TODO: Page size must be greater than the object size!!!!!!!!!!!
//                                                                                                                                                // NPTM::MEMGRANSIZE
template<size_t ObjLen, size_t PageLen=NPTM::MEMGRANSIZE, size_t FHdrLen=0, size_t NHdrLen=0> struct SBASLin: public SBASBase<AlignP2Frwd(PageLen,NPTM::MEMPAGESIZE), ObjLen, FHdrLen, NHdrLen, 6, (size_t)-1, SBASLin<ObjLen,PageLen,FHdrLen,NHdrLen> >   
{
 SCVR bool BestAlignPow2 = false;      // Multiple of 2 is enough
//-------------------------------------------------------------
static inline uint64 BlkIdxToSize(uint32 BlkIdx) {return PageSize * ++BlkIdx;}  
//-------------------------------------------------------------
static uint CalcForIndex(size_t ObjIdx, size_t& Idx)           // Cannot pregenerate a table - may be too big!!!
{
 SCVR size_t  PageBSize  = PageSize - NHdrSize;       // ??????????????????????
 SCVR ssize_t HdrLenDiff = NHdrSize - FHdrSize;       // Correction for FHdr size because leftover ranges precomputed for NHdr

 size_t uoffs = ObjIdx * ObjSize;          // GetOffsetForUnit(UnitIndex);
 size_t cbidx = ((size_t)NMATH::sqrt((8 * (uoffs / PageBSize)) + 1) - 1) / 2;   // FindBlockForOffset(uoffs);   // In a single block - Ignoring headers and leftover bytes (will correct for them later)
 size_t ucorr;   // TODO: Correct for first header
 if constexpr ((PageSize - NHdrSize) % ObjSize)   // Will need corrections not only for the first header
  {
 //  SCVR size_t UnitsOnPage = PageSize / ObjSize;     // ???????????????? unused???
   SCVR size_t BlkLftRange = ObjSize / ((size_t)1 << ctz(ObjSize));   // TODO: Test in loop!!!!!!!!!!!!!! // Leftover bytes sequence repeat range (in blocks)   // Same as "ObjSize / std::gcd(PageSize, ObjSize)" 

   struct SApArr    
   {
    using Base = typename Self;   // To fix dependent name lookup
    uint32 Arr[BlkLftRange+1];    // May be uint16?     // Biggest value is TotalInRange which is not known yet. uint32 should be enough 
    consteval SApArr(void) 
     {
      uint32 Total = 0;
      this->Arr[0] = Base::NHdrSize;
      for(size_t i=1; i <= BlkLftRange; i++)
       { 
        Total += ((Base::PageSize * i)-Base::NHdrSize) % Base::ObjSize;        // Must assume same header size for all blocks
   ...
   size_t corra = ((cbidx / BlkLftRange) * TotalInRange);   // Add first header fix here???????????
  //         size_t xxx = ((PageSize-FHdrSize) / ObjSize) * ObjSize;
 //  if(uoffs > xxx) 
  //   {
   ucorr  = corra + CorrArr.Arr[aidx];              // First block fheader fix needed?????????????????
   uoffs += corra + CorrArr.Arr[1+aidx];       // In a single block but with all extra bytes (headers+leftovers)   // Must recalc value from the array for first block
   //  }
   //   else
    //   {
    //     ucorr  = corra + 30;
     //    uoffs += corra + ((PageSize-FHdrSize) % ObjSize);
     //  }
     // if(uoffs >= UnitsOnPage)  
     //uoffs -= 30;
   // TODO: Correct for first header
   cbidx  = ((size_t)NMATH::sqrt((8 * (uoffs / PageSize)) + 1) - 1) / 2;     //FindBlockForOffset(uoffs);    // The square root again :(
   ////ucorr += cbidx * NHdrSize;
  }
   else
    {
     ucorr = 0;
 ...
 size_t cblen = PageSize * ((cbidx * (cbidx + 1)) / 2);          // GetCumulBytesForBlock
 size_t unfix = (uoffs - cblen) / ObjSize; //  (cblen - ucorr) / ObjSize;
 size_t ubidx = unfix;//ObjIdx - unfix; 
 Idx = ubidx;
 return cbidx;
}
//-------------------------------------------------------------
...
_finline SFHdr* GetCtx(void)  // _ninline
{
 ...
 return (SFHdr*)&this->Context;     // NOTE: Clang will not detect correctly all exit points in constexpr if-else and will go insane
}
//-------------------------------------------------------------
