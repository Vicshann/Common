...
SCVR bool SepBlkIdx = true;    // Block index array is a separate allocation (For strategies that require large indexes for blocks)
SCVR bool CtxInFBlk = true;    // The context is in the first block (Forced by MetaInBlk)
SCVR bool MetaInBlk = true;    // Metadata is added to every block (To find the block owner)  // TBIfo is nonzero or the flag is specified   // Forces context to be in the first block
...
// STASRT(GrowType <= afGrowGeo);
 enum {dbfDeleted=0x100,dbfEmpty=0x200,dbfAddrMsk=0xFFF};   // 4 bits max (Low byte is for a tag)

struct SBlkArrI    // Only for SBASGeom
{
 SCVR uint LMax = SBASGeom<0>::RangeNum;   // TODO: Take size from strategy
 size_t BlkNum;
 vptr   BlkArr[LMax];  

 inline void Init(void){}
 static inline size_t GetArrLen(void) {return sizeof(BlkArr)/sizeof(vptr);}
 inline vptr*  GetArrPtr(void) {return BlkArr;}
};

struct SBlkArrP
{
 SCVR uint LMax = 14;      // TODO: Take size from strategy  // Expected to be at least 0 (For first block) 
 ...
 vptr   Local[LMax];   // BlkArr Should point here initially

 inline void   Init(void){BlkArr = Local; BlkLen = LMax;}
 inline size_t GetArrLen(void) const {return BlkLen;}
 inline vptr*  GetArrPtr(void) {return BlkArr;}
};
...
struct SMNHdr { vptr Self; SFHdr* Ctx; };
...
//------------------------------------------------------------- 
int Release(void)              // !!! DONE      // Free everything, including tables and metadata if any
{
 ...
 if constexpr(SepBlkIdx)
  {
   if(BAPtr && (ctx->Blocks.GetArrLen() > ctx->Blocks.LMax) && (NPTM::NAPI::munmap(BAPtr, BASize * sizeof(vptr)) < 0))return -2;  // The block index was a separate allocation
  }
 memset(&this->Context,0,sizeof(this->Context));    // May be pointer or the context itself
 return 0;
}
//-------------------------------------------------------------              
int DeleteBlk(size_t BlkIdx)     // !!! DONE  // Deallocates a single block                 // !!!>>> How to find free blocks faster than iteration? How to know how many items will fit in a block?  // XOR pointers? (Of prev and next free blk index)
{
 SFHdr* ctx = this->GetCtx();
   ...
   if(BlkIdx)
    {
     ...
     else ctx->Self = BPtr;    // First block
  }
 vptr*  Adr = &ctx->Blocks.GetArrPtr()[BlkIdx];
 size_t Tag = (size_t)*Adr & (NPTM::MEMPAGESIZE - 1);
...
//-------------------------------------------------------------
Ty* GetBlkData(size_t BlkIdx, size_t BElmIdx)    // Leave all checks?    // BElmIdx is returned by GetRange  // Use this after GetRange to have all validations
{
...
//
static inline uint GetRange(size_t ElmIdx, size_t& BlkIdx, size_t& Size, size_t& Offs)   // Used by iterators, should be branchless   // !!!! BAAAADDDD
{
 Strat::CalcForIndex(ElmIdx, BlkIdx, ElmIdx);   // NOTE: No ElmIdx check. CalcForIndex may return nonsense!
...
static inline uint GetBlkEIdx(size_t& ElmIdx) {size_t BlkIdx; Strat::CalcForIndex(ElmIdx, BlkIdx, ElmIdx); return BlkIdx;}
//-------------------------------------------------------------
...
inline vptr   GetBlkPtr(size_t BlkIdx) const {return vptr((size_t)this->GetCtx()->Blocks.GetArrPtr()[BlkIdx] & ~((size_t)NPTM::MEMPAGESIZE - 1));}
inline uint32 GetBlkTag(size_t BlkIdx) const {return this->GetCtx()->Blocks.GetArrPtr()[BlkIdx] & (NPTM::MEMPAGESIZE - 1);}
inline void   SetBlkTag(size_t BlkIdx, uint32 Tag)      // It is important to have no checks if it is NULL or not
{
 ...
 size_t Addr = (size_t)*Ptr & ~((size_t)NPTM::MEMPAGESIZE - 1);
 *Ptr = vptr(Addr | (Tag & (NPTM::MEMPAGESIZE - 1)));
}
//-------------------------------------------------------------
