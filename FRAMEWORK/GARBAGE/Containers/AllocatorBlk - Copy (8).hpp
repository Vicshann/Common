...
SCVR size_t _AlUnitLen  = (Flg & afObjAlign)?(TBStrat::BestAlignPow2?(AlignToP2Up(sizeof(Ty))):(AlignP2Frwd(sizeof(Ty), sizeof(size_t)))):(sizeof(Ty));   // May align to nearest Pow@ or by pointer size, depending what is the best for the current strategy
SCVR size_t _AlNHdrLen = sizeof(SNHdr) ? ( (_AlUnitLen < 64)?(AlignFrwd(sizeof(SNHdr),_AlUnitLen)):(AlignP2Frwd(sizeof(SNHdr),64)) ):0;              // May align to UnitSize  // Alignment 64 is enough even for AVX512
SCVR size_t _AlFHdrLen = CtxInFBlk ? ( (_AlUnitLen < 64)?(AlignFrwd(sizeof(SFHdrEx),_AlUnitLen)):(AlignP2Frwd(sizeof(SFHdrEx),64)) ): _AlNHdrLen;
...
public:
 ...
 {
  this->SeekTo(pos * Strat::UnitSize);
 }
 //------------------------------------------------------ 
 // Prefix 
...
SIterator ElmFrom(size_t ElmIdx)   
{
 ...
 size_t ElmOffs = ElmIdx * Strat::UnitSize;
 ...
 for(sint idx=ctx->GetArrLen()-1;idx >= 0;idx--)
  {
   ...
   if(ptr)return SIterator(ptr, idx, (size_t)Strat::BlkIdxToSize(idx) - Strat::UnitSize, ctx);   // TODO: Calc offset for last index
  }
 return SIterator(ctx); 
}
//------------------------------------------------------------- 
