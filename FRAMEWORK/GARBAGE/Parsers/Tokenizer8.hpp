...
enum ETokenFlags {   // Tokenizer flags, not a token flags  // Token flags or range flags?   // Only 8 bits for token flags!   // This is for Range/Token records, not the token stream itself
  tfNone        = 0, 
  tfWhtspc      = 0x000001,   // This range is a whitespace
  tfIgnore      = 0x000002,   // Ignore the char range, like ' in numbers. Do not change the state?
  tfNoTerm      = 0x000004,   // The char range is a part of a token but cannot be last in it
  tfBadToken    = 0x000008,   // Treat unspecified chars as an error instead of fallback to state 0
  tfScopeOpn    = 0x000010,   // Both flags mean that same symbols wil open and close their scope (i.e.: ' " )   They will use a separate scope stack ???          //rfScopeOpn + rfScopeCse for counted scopes like ''' or '"'   // Note Do not allow scope close if it is not opened
  tfScopeCse    = 0x000020,   // Can we have nested scopes of same char?  NO!: "1 "2 "3 "?[We want to close the last one]    // So, the flat only: "O "C "O "C
  tfNumeric     = 0x000040,   // No lexing. Store in a separate list with associated struct of converted values to do conversion only once
  tfTknTerm     = 0x000080,   // This token terminates/separates an expression (i.e. ';')
// Not transferable to token records
  tfTknRSplit   = 0x000100,   // Split current token when encountered // Will become part of a next token, not the current one (Not same as tfTknTerm?)
  tfTknLSplit   = 0x000200,   // Split current token when encountered // Will become part of the current token
  tfRawString   = 0x000400,   // A string with no escape processing
  tfEscString   = 0x000800,   // An escaped string
  tfComment     = 0x001000,
  tfLSplitOnSDZ = 0x002000,   // Split if scope depth of GroupID becomes zero after tfScopeCse (Does tfTknLSplit)
  tfBRstIfSplit = 0x004000,   // Reset current state to base if split
  fScpTknMirror = 0x008000,   // This token is a mirrored when used as a scope marker (Store the token to compare on closing the scope)  // Format: [Prefix]anything[Postfix] Prefix/Postfix is controlled by ranges and 'anything' is stored on scope stack to compare on close   // C++: R "delimiter( raw_characters )delimiter"     // Delimiter is optional and it can be a character except () / or whitespaces
  tfTknMemStore = 0x010000,   // Remember token accumulated before the current char (type flags accumulation will continue)
  tfTknMemSplit = 0x020000,   // Split at a remembered token
};
...
struct SRange
{
 uint8  First;    
 uint8  Last;
 uint16 State;      // Max 256 states(Max uint8 value 00-FF), 64K memory(Allocated by 4K)
...
//------------------------------------------------------
sint Upd(const achar* TxtPtr, uint8 GroupId, uint8 Flags, SPos&& TknPos, SPos* ScOpnPos=nullptr)
{
...
void UpdateToken(uint8 Chr, SRangeLst::SRange* Rng)
{
 ...
 if(!(Rng->Flags & tfIgnore))TokenBuf[this->CurTkn.Pos.Size++] = Chr;   // Accumulate a token 
}
//------------------------------------------------------------------------------------------------------------
sint ProcessToken(uint8 CurState, SPos& PrvPos)
{
// DBGMSG("Token: '%.*s'",TknLen, TknVal);
 if(!this->CurTkn.Pos.Size){this->ResetToken(); return 0;}

 PrvPos.Set(&this->CurTkn.Pos);
 if(this->CurTkn.Flags & tfNoTerm)
  return -tleChrCantBeLast;         // A last char brought this flag and it is not allowed to be last

 if((this->CurTkn.Flags & tfWhtspc) && !(this->ParseFlg & pfKeepWhtspc)){this->ResetToken(); return 0;}  // Ignore whitespaces
 DBGMSG("Token %u: '%.*s'",CurState, this->CurTkn.Pos.Size, &TokenBuf);
...
//
sint AddRange(uint64 TgtState, uint8 First, uint8 Last, uint8 NextState, uint32 Group=0, uint32 TypeSet=0, uint32 TypeClr=0, uint32 Flags=0)
{
...
// We have some valid range in Rng now         // !!! Need support for single chars splitted
   ...
   if(Rng->Flags & (tfScopeOpn|tfScopeCse))   // Can do scoping even in the middle of a token (Scope closing does not imply splitting)    // TODO: Apply only to an fully accumulated token  // How? We need splitting to be done for that
    {
     if(sint res=this->Scopes.Upd(this->BegPtr, Rng->Group, Rng->Flags, SPos{LineNum, LinePos, uint(CurPtr-BegPtr), this->CurTkn.Pos.Size}, &PrvPos);res < 0)    // At this point a last token char is not yet appended and even CurTkn may be a last token
      {
       if(ErrCtx)
        {
       ...
       else if(!res && (Rng->Flags & (tfScopeCse|tfLSplitOnSDZ)) == (tfScopeCse|tfLSplitOnSDZ))SplitFlg |= tfTknLSplit;     // LSplit
    }
   ...
   if(!LSplit)  // Just update the current token
    {
     if(SplitFlg & tfTknMemStore)this->MemTkn = this->CurTkn;   // The current char will not be a part of the remembered token   // Is here the best place for this?
     this->UpdateToken(Val, Rng);
    }
