...
enum ERangeFlags {
  rfNone    = 0, 
  rfIgnore  = 0x0001,   // Ignore the char range, like ' in numbers. Unused - Just do not change the state
  rfBadChar = 0x0002,   // Treat unspecified chars as an error instead of fallback to state 0
};
...
struct SRange
{
 ...
 uint8  Last;
 uint8  State;
 uint8  Group;      // Group?   // By default all unassigned char ranges cause fallback to state 0. Invalid char ranges must be defined to break parsing if necessary 
 uint16 TypeSet;    // Set accumumulated token flags for the Lexer (Low part)
 uint16 TypeClr;    // Clear accumumulated token flags for the Lexer (Low part)
   ...
   if(uint ridx = RangeMap[idx];ridx)   // Already belongs to some range   // We can write ranges inside of ranges but without overlapped borders
    {
     ...
     if(rec.Group <= Rng->Group)              // A higher group will always overwrite       // TODO: Remove group property?
      {
...
//
sint AddRange(uint8 TgtState, uint8 First, uint8 Last, uint8 NextState, uint16 Type=0, uint16 Flags=0)
{
 SRangeLst::SRange rng{.First=First, .Last=Last, .Group=0, .State=NextState, .Type=Type, .Flags=Flags};
...
Repeat:       
   if(SRangeLst::SRange* Rng = this->Ranges.Get(Val, State);!Rng->IsValid())   // Fallback and split        // NOTE: It is possible to use ranges to parse some multibyte UTF-8 chars
    {
     ...
     if(TknSize)   // Skip whitespace tokens (Optionally)
      {
       LOGMSG("Token: '%.*s'",TknSize, &TokenBuf);
     //STokenLst::ADD
      }
   ...
   TokenBuf[TknSize++] = Val;   // Accumulate a token
   LinePos = LineNxt;
   CurPtr++;
   ChrLen--;
  }
 if(TknSize)  // Add a last token // Skip whitespace tokens (Optionally)     // Only if fallback is allowed at this point else report an unexpected EOF
  {
   LOGMSG("Last Token: '%.*s'",TknSize, &TokenBuf);
 //STokenLst::ADD
  }
