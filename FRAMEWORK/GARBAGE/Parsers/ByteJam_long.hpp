...
{
static constexpr uint32 MaxDist = 0x1FFFFF;
enum EBFlags    
{
 flSeqEnd  = 0x80,  // This bit ends some word
 flOIndex  = 0x40,  // The offset value is actuallyindex in array of indexes
 flExOffs  = 0x20,  // No offs if next rec is adjacent  // Of have offs ant it is 0 then this is an terminal rec (Offset is always from current rec by words)
 flOffsMsk = 0x1F,  // Max index is 4194303
};
//==================================================================================
...
CArr<uint32> Offset;   // Extended offsets    // NOTE: The offsets are used a lot if the word list is passed unsorted
//------------------------------------------------------------------------------------------------------------
...
{
 uint16 CfgOr = (bit->Flags & (flSeqEnd|flOIndex));    // Only flSeqEnd|flOIndex and only set is supported
 ...
 while(WBits > 0)  // Counting the bits is faster     
  {
   ...
   if(bv->IsTerm())     // This bit is last on the path
      ...
      *LastBit = bv;
      return 1;     // The word is longer than the path
     }
...
// Continue storing the word at updated by MatchWordAt position (The word is longer than the path) 
 ...
 if(bv)    // Last present bit is terminal and distance to a next bit must be set
  { 
   ...
   if((uint32)NextOffs > MaxDist)          // Failed to store (Should store elsewhere)
    {
     ...
     if((uint32)NextOffs >= MaxDist){LOGERR("Offset index overflow!"); return -2;}
 ...
 while(WBits > 0)    // Store new bits
  {
   ...
   bz->Offs  = 0;
   WBits--;
   if(!WBits){ bv->Offs = 0; bv->Flags |= flExOffs; Wasted++;}  // Word Terminal bit positions should have an extended offsets on both paths (End of batch)  (Storing longest words first may save some space)
