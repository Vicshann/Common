...
//------------------------------------------------------
/*sint Inc(uint8 GroupId)       // Unrelated to actual scope stack (Useful for multiline comment nesting)
{
 uint8* DepthPtr = &this->DepthState[GroupId];
 uint prv = *DepthPtr;
 if(prv == (MaxScopes-1))return -tleScopeTooDeep;
 (*DepthPtr)++;
 return prv;
} */
//------------------------------------------------------
...
// DBGMSG("Token: '%.*s'",TknLen, TknVal);
 ...
 DBGMSG("Token %u: '%.*s'",CurState, TknLen, TknVal);
...
// Conflict: Explicit splitting (which should move to a next char) conflicts with a fallback splitting which should eval same char   //                                         
// If in current state the char is not in defined range, try a next state (And if the next state is same - reset the state to base)
// What to do when the char is not in current state`s defined ranges?   Go to a next state or to a base state?   // If a range is not defined it is a empty record with NextState is 0 (Base state)
Repeat:    
   if(SRangeLst::SRange* Rng = this->Ranges.Get(Val, State);!Rng->IsValid() || DoSplit || (DoSplit=(Rng->Flags & tfTknLSplit), (Rng->Flags & tfTknRSplit)))   // Fallback and split    // NOTE: It is possible to use ranges to parse some multibyte UTF-8 chars
    {
     if(!State && !DoSplit)   // Nowhere to fallback, already in state 0 (All usable ASCII ranges must be defined in state 0)
      {
       if(Rng->Flags & tfTknRSplit){DoSplit |= true; goto Next;}     // Allow to split right in the state   // NOTE: Invalid range is just an empty range record
...
//       DBGMSG("Token: '%.*s'",TknSize, &TokenBuf);
     ...
     if(State == Rng->State)goto Next;   // To prevent an infinite loop  // DoSplit ?
     State   = Rng->State;    // Invalid ranges split to Base state 0
        ...
        if(sint res=this->Scopes.Upd(Rng->Group, Rng->Flags, LineNum, LinePos, uint(CurPtr-BegPtr), &PrvPos);res < 0)
         {
          ...
          else DoSplit |= (!res && (Rng->Flags & (tfScopeCse|tfLSplitOnSDZ)) == (tfScopeCse|tfLSplitOnSDZ));
       }
