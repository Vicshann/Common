...
// Not transferable to scope records
...
//============================================================================================================
struct STknMem   // Curcular buffer
{
 SCVR uint MaxTokens = 128;   // Max tokens to remember   // Should be pow2
 uint LastPosInBuf;
 CArr<STknInf> Tokens;

STknMem(void){this->LastPosInBuf = MaxTokens-1;}
uint Store(const STknInf& tkn)
{
 this->LastPosInBuf = (this->LastPosInBuf + 1) & (MaxTokens-1);
 if(this->Tokens.Count() < MaxTokens)this->Tokens.Append(&tkn);
  else memcpy(this->Tokens.Get(this->LastPosInBuf), &tkn, sizeof(STknInf));
}
//------------------------------------------------------
STknInf* Get(bool Pop=true)
{
 if(!this->Tokens.Count())return nullptr;   // Report an error in this case (Possibly an incorrect state definition)
 STknInf* ptr = this->Tokens.Get(this->LastPosInBuf);
 if(Pop)
  {
   if(!this->LastPosInBuf)this->LastPosInBuf = MaxTokens-1;
     else this->LastPosInBuf--;
  }
 return ptr;
}
};
...
// We have some valid range in Rng now         // !!! Need support for single chars splitted
   ...
   if(Rng->Flags & (tfScopeOpn|tfScopeCse))   // Can do scoping even in the middle of a token
    {
     if(sint res=this->Scopes.Upd(this->BegPtr, Rng->Group, Rng->Flags, SPos{LineNum, LinePos, uint(CurPtr-BegPtr), this->CurTkn.Pos.Size}, &PrvPos);res < 0)
      {
       if(ErrCtx)
        {
