...
// Consumes ~8 times more memory than original data!  ( But it is sparse and very fast at finding matches )  (Have no limit on a word size and consumes much less memory than a char table would do)
//------------------------------------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------------------------------------
sint8 StoreWordAt(uint32& At, SBits& Word)    // Just returns same offset if the word was already stored    // NOTE: Do not call on an empty stream or an empty word!
{
 SBitNode Bitn;
 SBit* bv = nullptr;
