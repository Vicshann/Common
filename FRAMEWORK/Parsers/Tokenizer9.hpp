...
// Not transferable to token records
  ...
  tfScpMrkBegin = 0x040000,   // Next char will start a scope marker
  tfScpMrkEnd   = 0x080000,   // This char ends a scope marker(Not becoming a part of it if not marked with tfScpMrkBegin too)
};
 ...
 if(Flags & tfScopeCse)
  {
   ...
   if(ScpMrk.Size != Last->Mrk.Size)
      return -tleScpMrkMismatch;   // Marker mismatch error
...
// DBGMSG("Token: '%.*s'",TknLen, TknVal);
 ...
 DBGMSG("Token %u: '%.*s'",CurState, Token.Pos.Size, &TokenBuf);
   ...
   if(SplitFlg & (tfScopeOpn|tfScopeCse))   // Can do scoping even in the middle of a token (Scope closing does not imply splitting)    // TODO: Apply only to an fully accumulated token  // How? We need splitting to be done for that
    {
     ...
     if((res == -tleScpMrkMismatch)&&(SplitFlg & tfRawString))  // Mismatched raw string close marker, should continue (A hack: remove all flags that expected to be at the real raw string end)
      {

       SplitFlg &= ~();
      }
     else if(res < 0)    // At this point a last token char is not yet appended and even CurTkn may be a last token
      {
       ...
       else if(!res && (SplitFlg & (tfScopeCse|tfLSplitOnSDZ)) == (tfScopeCse|tfLSplitOnSDZ))SplitFlg |= tfTknLSplit;     // LSplit
//     if(smrk.Size && (SplitFlg & tfScopeCse))smrk.Offs = smrk.Size = 0;   // Reset scope marker ?
    }
 ...
 if(sint res=this->ProcessToken(State, this->CurTkn, PrvPos);res < 0)
  {
   if(ErrCtx)ErrCtx->Set(-res, State, this->CurTkn.Pos.Size, TokenBuf, SPos{LineNum,LinePos,uint(CurPtr-BegPtr),this->CurTkn.Pos.Size}, SPos{PrvPos});
   return res;
  }
