...
// key length and block length are possible). Both block length and key length can be extended very easily to multiples of 32 bits.
// Rijndael can be implemented very efficiently on a wide range of processors and in hardware.
// This implementation is based on the Java Implementation used with the Cryptix toolkit found at: http://www.esat.kuleuven.ac.be/~rijmen/rijndael/rijndael.zip
...
//
// C++17 required
// NOTE: Nonstatic version occupies ~12k in BSS section
//
class CRijndael
{
protected:
 ...
 if(CBC == iMode) // CBC mode, using the Chain
  {
   if(in == out)
    {
     unsigned char Temp[MAX_BLOCK_SIZE];
     for(size_t i=0,e=n/BlockSize;i < e; i++, in += BlockSize, out += BlockSize)
      {
