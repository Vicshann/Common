...
//------------------------------------------------------------------------------------------------------------
template<uint32 StratType, size_t UnitLen=0, size_t PageLen=0, size_t FHdrLen=0, size_t NHdrLen=0> struct SSel
{
 STASRT(StratType <= afGrowExp, "Wrong grow strategy!");
 using T = TSW<StratType==afGrowExp,SBASExp<UnitLen,PageLen,FHdrLen,NHdrLen>,TSW<StratType==afGrowLin,SBASLin<UnitLen,PageLen,FHdrLen,NHdrLen>,SBASUni<UnitLen,PageLen,FHdrLen,NHdrLen> >::T>::T;
};
//============================================================================================================
//                                          The Allocator
//------------------------------------------------------------------------------------------------------------
...
//
template<size_t PageLen, uint32 Flg, typename Ty=uint8, typename TCIfo=SEmptyType, typename TBIfo=SEmptyType, typename MP=SMemPrvBase, size_t MaxAlign=64> class SBlkAlloc   // Multiple linear ranges    // Alignment 64 is enough even for AVX512
{
...
SCVR size_t AlNHdrLen = sizeof(SNHdr) ? ( (!(Flg & afLimitHdrAl) || (AlUnitLen <= MaxAlign))?(AlignFrwd(sizeof(SNHdr),AlUnitLen)):(AlignP2Frwd(sizeof(SNHdr),MaxAlign)) ):0;              // May align to UnitSize  
...
class SIterator    // I do not like clumsy ranged FOR but i need iterator object to hide iteration between blocks. :: for (auto it = alc.begin(), end = alc.end(); it != end; ++it) {const auto i = *it; ... }
{
 ...
 void SetBlockFor(size_t ElmIdx)      // NOTE: Will not check if the block is NULL     // TODO: Test it!!!
 {
  ...
  size_t HSize  = Strat::BlkIdxToHdrLen(BlkIdx);
  vptr*   APtr  = this->Ctx->GetArrPtr();
  size_t  blkp  = (size_t)APtr[BlkIdx] & Strat::PtrMask;
  this->EIdx = ElmIdx;
